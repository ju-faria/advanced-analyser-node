(function (global, factory) {
	...
	*/
	...
	const useSpectrogramRenderer = ({ canvas, dataResolver, frequencyScale = constants.exports.FrequencyScale.logarithmic, }, deps) => {
	    ...
	    react.exports.useEffect(() => {
	        ...
	        const dataUpdateHandler = (event) => {
	            if (!spectrogramRenderer)
	                return;
	            if (!spectrogramRenderer.isFrequencyBinVisible(event.detail.frequencyBinIndex))
	                return;
	            spectrogramRenderer.draw();
	        };
	    ...
	    return {
	        ...
	        updateProperties: ({ maxFrequency, minFrequency, currentTime, timeWindow }) => {
	            if (!spectrogramRenderer)
	                return;
	...
	const useImmutableRef = (initialValue) => {
	    const [current, setCurrent] = react.exports.useState(initialValue);
	...
	const SpectrogramContext = React.createContext({
	    ...
	    transforms: {
	        ...
	        maxFrequency: constants.exports.DEFAULT_MAX_FREQUENCY,
	        timeWindow: constants.exports.DEFAULT_TIME_WINDOW,
	        currentTime: 0,
	    },
	    width: 1024,
	    height: 512,
	...
	*/
	...
	const translateCurrentTime = (timeAxisSize, delta) => (properties) => {
	    const { timeWindow, currentTime, } = properties;
	    const timePerPixel = (timeWindow / timeAxisSize);
	    return { ...properties, currentTime: Math.max(currentTime - (timePerPixel * delta), 0) };
	};
	const scaleTimeWindow = (scaleFactor, origin) => (properties) => {
	    ...
	    const newTimeWindow = lodash.exports.clamp(timeWindow * scaleFactor, 1000, 3600_000);
	...
	const useAnimationFrame = (callback) => {
	    const requestRef = react.exports.useRef();
	    const previousTimeRef = react.exports.useRef(performance.now());
	    const animate = () => {
	...
	const useGestures = (ref, { onPanMove, onPanStart, onPanEnd, onPinchMove, onPinchStart, onPinchEnd, onWheel, onTrackpadPinch, }) => {
	...
	const useControls = ({ modifierKeyCode = 'ShiftLeft', lockFrequencyPanning = false, lockTimePanning = false, lockFrequencyScaling = false, lockTimeWindowScaling = false, onChange, width, height, canvas, transforms, frequencyScale = defaults.DEFAULT_FREQUENCY_SCALE, }) => {
	    ...
	    useGestures(canvas, {
	        ...
	        onPinchMove: (e) => {
	            ...
	            const transformed = composeTransforms(translateCurrentTime(width, e.deltaX), translateFrequency(height, e.deltaY), scaleTimeWindow(Math.abs(Math.max(e.pinchStartMagnitudeX, 40)
	                /
	                    Math.max(e.pinchMagnitudeX, 40)), e.relativeX / width), scaleFrequencies(Math.abs(Math.max(e.pinchStartMagnitudeY, 40)
	                /
	        ...
	        onPanStart: () => {
	            if (lockFrequencyPanning && lockTimePanning)
	                return;
	            setActivePanEvent({
	                startFrequencies: {
	                    min: minFrequency,
	        ...
	        onPanMove: (e) => {
	            if (activePanEvent) {
	                ...
	                if (!lockFrequencyPanning) {
	                    transformed = composeTransforms(translateFrequency(height, e.y - e.startY))(transformed);
	                }
	                if (!lockTimePanning) {
	                    transformed = composeTransforms(translateCurrentTime(width, e.x - e.startX))(transformed);
	                }
	                onChange(transformed);
	            }
	        },
	        ...
	        onWheel: (e) => {
	            ...
	            if ((modifierKeyCode === null || modifierIsPressed)
	                &&
	                    !lockFrequencyScaling) {
	                const delta = (height + (e.deltaWheel * -1)) / height;
	                transformed = composeTransforms(scaleFrequencies(delta, e.relativeY / height))(transformed);
	            }
	            if ((modifierKeyCode === null || !modifierIsPressed)
	                &&
	                    !lockTimeWindowScaling) {
	                const delta = (width + (e.deltaWheel * -1)) / width;
	                transformed = composeTransforms(scaleTimeWindow(delta, e.relativeX / width))(transformed);
	            }
	            onChange(transformed);
	        },
	        onTrackpadPinch: (e) => {
	    ...
	    useAnimationFrame(() => {
	        ...
	        if (!lockTimePanning) {
	            transformed = composeTransforms(translateCurrentTime(width, releaseDeltaPosition.x + newDeltaPosition.x))(transformed);
	        }
	        if (!lockFrequencyPanning) {
	            transformed = composeTransforms(translateFrequency(height, releaseDeltaPosition.y + newDeltaPosition.y))(transformed);
	        }
	        onChange(transformed);
	...
	const Spectrogram = ({ width, height, frequencyScale = constants.exports.DEFAULT_FREQUENCY_SCALE, dynamicRange, dynamicRangeTop, modifierKeyCode = 'ShiftLeft', dataResolver, transforms, displayFrequencyRuler = true, frequencyRulerAsOverlay = true, frequencyRulerPosition = 'start', frequencyRulerSize = 50, displayTimeRuler = true, timeRulerAsOverlay = true, timeRulerPosition = 'end', timeRulerSize = 30, onChange, children, ...props }) => {
	    ...
	    return (React.createElement("div", { style: {
	        ...
	        React.createElement("canvas", { ref: canvasRef, width: canvasWidth, height: canvasHeight, style: {
	                touchAction: 'none',
	                marginTop: canvasTopOffset,
	                marginLeft: canvasLeftOffset,
	            } }),
	        displayFrequencyRuler && (React.createElement(FrequencyRuler, { ref: frequencyRulerRef, width: frequencyRulerSize, height: canvasHeight, minFrequency: transforms.minFrequency, maxFrequency: transforms.maxFrequency, position: frequencyRulerPosition === 'start' ? 'inset' : 'offset', frequencyScale: frequencyScale, style: {
	                position: 'absolute',
	                top: canvasTopOffset,
	                [frequencyRulerPosition === 'start' ? 'left' : 'right']: 0,
	            } })),
	        displayTimeRuler && (React.createElement(TimeRuler, { ref: timeRulerRef, width: canvasWidth, height: timeRulerSize, timeWindow: transforms.timeWindow, currentTime: transforms.currentTime, position: timeRulerPosition === 'start' ? 'inset' : 'offset', style: {
	                position: 'absolute',
	                bottom: 0,
	...
	const App = () => {
	    ...
	    const [spectrogramTransforms, setSpectrogramTransforms] = React.useState({
	        minFrequency: 20,
	        maxFrequency: 44100,
	        timeWindow: 10_000,
	        currentTime: 0,
	    ...
	    const aaNode = useAsyncMemo(() => {
	        if (!offlineCtx)
	            return null;
	        return advancedAnalyserNode.createAdvancedAnalyserNode(offlineCtx, {
	            samplesBetweenTransforms: offlineCtx.sampleRate / 12
	        });
	    }, [offlineCtx]);
	    const dataResolver = react.exports.useMemo(() => {
	        if (!aaNode)
	            return null;
	        return new bundle.exports.DefaultFrequencyDataResolver({
	    ...
	    return (React.createElement("div", null,
	        dataResolver && (React.createElement(Spectrogram, { width: width, height: height - 20, transforms: spectrogramTransforms, onChange: setSpectrogramTransforms, dynamicRange: dynamicRange, dynamicRangeTop: dynamicRangeTop, onDynamicRangeChange: setDynamicRange, onDynamicRangeTopChange: setDynamicRangeTop, dataResolver: dataResolver, timeRulerAsOverlay: false, frequencyRulerAsOverlay: false, frequencyRulerPosition: frequencyRulerPosition, timeRulerPosition: timeRulerPosition, frequencyScale: frequencyScale },
	            React.createElement("span", { className: "playhead", style: {
	                    display: playheadPosition > spectrogramTransforms.currentTime
	                        ...
	                        ? 'block' : 'none',
	                    transform: `translateX(${(width - 50) * ((playheadPosition - spectrogramTransforms.currentTime) / spectrogramTransforms.timeWindow) + (frequencyRulerPosition === 'start' ? 50 : 0)}px)`,
	                } }))),
